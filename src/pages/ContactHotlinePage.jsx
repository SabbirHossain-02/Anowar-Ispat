...
const TONES = ['#E3182D', '#22c55e', '#3b82f6', '#a855f7', '#eab308'];
...
const DEFAULTS = {
    hero: {
        ...
        title: 'Hotline /',
        accent: 'Email',
        sub: 'Get in touch with our team directly via phone, email, or social media channels.',
    },
    phones: [
  {label:'Sales Hotline', number:'+880 2-XXXX-XXXX', hours:'Sun–Thu · 9:00 AM – 6:00 PM', desc:'For product inquiries, quotes, and sales support.' },
  {label:'Factory / Technical', number:'+880 1XXX-XXXXXX', hours:'24/7 Emergency Line', desc:'For technical support, factory operations and urgent matters.' },
  {label:'Corporate Office', number:'+880 2-XXXX-XXXX', hours:'Sun–Thu · 9:00 AM – 5:00 PM', desc:'For corporate affairs, partnerships and general inquiries.' },
],
    emails: [
  {label:'General Enquiry', email:'info@anwarispat.com', desc:'For all general inquiries and information requests.' },
  {label:'Media / PR', email:'media@anwarispat.com', desc:'For press, media coverage and public relations.' },
  {label:'Sales & Orders', email:'sales@anwarispat.com', desc:'For product orders, pricing and sales inquiries.' },
  {label:'Careers', email:'careers@anwarispat.com', desc:'For job applications and recruitment inquiries.' },
],
    socials: [
  { name:'Facebook',handle:'@AnwarIspat', href:'https://facebook.com' },
  { name:'LinkedIn',handle:'Anwar Ispat Ltd.', href:'https://linkedin.com' },
  { name:'YouTube',handle:'@AnwarIspatOfficial', href:'https://youtube.com' },
],
};

const ContactHotlinePage = () => {
  const containerRef = useRef(null);
  const c = useContent('contact-hotline', DEFAULTS);
    ...
    <div ref={containerRef} style={{ background:'var(--primary)', color:'var(--text)', minHeight:'100vh', paddingTop:'80px', overflowX:'hidden' }}>
      ...
      <section style={{ padding:isMobile?'16px 24px 16px':'20px 40px 16px', borderBottom:'1px solid var(--glass-border)', position:'relative', overflow:'hidden', textAlign:'center' }}>
        ...
        <div style={{ maxWidth:'860px', margin:'0 auto' }}>
          <div className="ch-hero-tag" style={{ fontSize:'10px', letterSpacing:'4px', color:'var(--accent)', textTransform:'uppercase', marginBottom:'14px' }}>{c.hero.tag}</div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'10px', marginBottom:'40px' }}>
          {c.phones.map((p, i) => (
            <a key={i} href={'tel:' + p.number} className="ch-card"
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor=tone(i) + '50'; e.currentTarget.style.background=tone(i) + '06'; e.currentTarget.style.transform='translateX(4px)'; }}
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:'10px', letterSpacing:'2px', color:tone(i), textTransform:'uppercase', marginBottom:'5px', fontWeight:700 }}>{p.label}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:'10px', marginBottom:'40px' }}>
          {c.emails.map((em, i) => (
            <a key={i} href={'mailto:' + em.email} className="ch-card"
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor=tone(i) + '50'; e.currentTarget.style.background=tone(i) + '06'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor='var(--glass-border)'; e.currentTarget.style.background='var(--glass)'; }}>
              <div style={{ width:'44px', height:'44px', borderRadius:'12px', background:tone(i)+'1a', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:'9px', letterSpacing:'2px', color:tone(i), textTransform:'uppercase', marginBottom:'5px', fontWeight:700 }}>{em.label}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)', gap:'10px' }}>
          {c.socials.map((s, i) => (
            <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" className="ch-card"
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor=tone(i) + '50'; e.currentTarget.style.background=tone(i) + '08'; e.currentTarget.style.transform='translateY(-2px)'; }}
